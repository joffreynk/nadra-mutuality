...
export default function MedicineRequests() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch('/api/pharmacy/my-requests');
          ...
          <Card key={req.id}>
            <CardContent className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              ...
              <div className="flex items-center gap-2 mt-2 sm:mt-0">
                <div className="text-sm text-gray-600 mr-4">{(req.pharmacyRequests?.length ?? 0)} item{(req.pharmacyRequests?.length === 1 ? '' : 's')}</div>
